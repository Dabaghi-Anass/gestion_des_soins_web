...
export async function toDataUrl(url: string) {
	try {
		...
		throw new Error(
			`Failed to fetch or convert URL to data URL: ${error.message}`
		);
	}
}
...
export function getBadgeStyle(status: string, withBorder: boolean = false) {
	...
	let style = {
		...
		border: withBorder
			? `1px solid ${getColorOfHue(getStatusHue(status))}`
			: "none",
	};
	return style;
}
...
export function randomTileWindColor() {
	...
	return tailwindColors[
		Math.floor(Math.random() * tailwindColors.length - 1)
	];
}
...
export function getGenderName(gender: string = "male") {
	return gender?.toLowerCase() === "male" ? "homme" : "femme";
}
