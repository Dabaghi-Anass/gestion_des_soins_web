...
const getRequestTreatments = async (userId: number) => {
	try {
		...
		const response: any = await http.get(
			`${TREATMENTS_URL}/requests/${userId}`
		);
		if (response.ok) {
			return response.json();
		}
...
const updateRequestStatus = async (id: number, status: string) => {
	try {
		if (id === null || id === undefined) return null;
		const response: any = await http.put(`${TREATMENTS_URL}/status/${id}`, {
			status,
		});
		if (response.ok) {
			return response.json();
...
const getTreatmentsByUserId = async (id: number, offset = 0, limit = 6) => {
	try {
		if (id === null || id === undefined) return null;
		const response: any = await http.get(
			`${TREATMENTS_URL}/${id}?offset=${offset}&limit=${limit}`
		);
		if (response.ok) {
			return response.json();
		}
...
const getTreatmentByRequestId = async (id: number) => {
	try {
		if (id === null || id === undefined) return null;
		const response: any = await http.get(
			`${TREATMENTS_URL}/treatment-by-request/${id}`
		);
		if (response.ok) {
			return response.json();
		}
...
const updateTreatment = async (treatment: any) => {
	try {
		const response: any = await http.put(
			`${TREATMENTS_URL}/update-treatment`,
			treatment
		);
		if (response.ok) {
			return response.json();
...
async function getActivityRequests(userId: number, offset: number, limit = 5) {
	try {
		const response = await http.get(
			`${ACTIVITY_URL}?userId=${userId}&offset=${offset}&limit=${limit}`
		);
		if (response.ok) {
			return response.json();
		}
...
async function getAppointmentRequests(
	...
	try {
		const response = await http.get(
			`${APPOINTMENT_URL}?userId=${userId}&offset=${offset}&limit=${limit}`
		);
		if (response.ok) {
			return response.json();
		}
...
async function uploadImage(user_id: number, image: any) {
	try {
		...
		const response = await http.postFile(
			`${STORAGE_URL}/upload-image/${user_id}`,
			data
		);
		if (response.ok) {
...
async function getAllUserDocuments(user_id: number) {
	try {
		if (!user_id && user_id !== 0) return null;
		const response = await http.get(
			`${STORAGE_URL}/user-files/all/${user_id}`
		);
		if (response.ok) {
			return response.json();
		}
